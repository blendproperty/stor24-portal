...
import { guideStorageKey, helpForPath, parseGuidePreferences, reviewGuideStep, workflowGuides, type GuidePreferences, type GuideStep } from "@/lib/guided-help";
...
export function GuidedHelp({ userId }: { userId: string }) {
  ...
  const onStepPage = step?.route === pathname;
      ...
      <aside id="guided-help-panel" className="guide-panel" aria-label="STOR24 guided help">
        ...
        <div className="guide-panel-scroll">
          ...
          {screenView === "library" && <>
            ...
            <div className="guide-library">{workflowGuides.map((item, number) => {
              const reviewed = preferences.progress[item.id]?.reviewed.length ?? 0;
              return <button type="button" key={item.id} className="guide-card" onClick={() => start(item.id)}><span className="guide-card-number">{reviewed === item.steps.length ? <Check size={18} /> : `0${number + 1}`}</span><span><strong>{item.title}</strong><span className="guide-card-description">{item.description}</span><small>{item.duration} · {item.steps.length} steps{reviewed > 0 ? ` · ${reviewed} read` : ""}</small></span><ArrowRight size={17} /></button>;
            })}</div>
