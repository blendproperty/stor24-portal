  ...
  return <main className="tenant-portal">
    ...
    <div className="tenant-body">
      {checking ? <p role="status">Opening your secure space…</p> : !data ? <div className="tenant-entry">
        ...
        {selectedUnit && <section className="tenant-card"><h2>Packing supplies for Unit {selectedUnit.number}</h2><p>Your recorded package selections. Reserved items are not proof of payment or delivery.</p>{selectedUnit.reservations.map(reservation => reservation.packageSelection && <div className="tenant-card" key={reservation.id}><h3>{reservation.packageSelection.packageName}</h3><p>{reservation.packageSelection.status.replaceAll("_", " ")} · {currency(reservation.packageSelection.priceSnapshot)} · {reservation.publicReference}</p><ul>{packageItems(reservation.packageSelection.itemsSnapshot).map((item, index) => <li key={index}>{item.quantity} × {item.name}</li>)}</ul>{reservation.packageSelection.fulfilledAt && <p>Fulfilled {formatSouthAfricaDate(reservation.packageSelection.fulfilledAt)}</p>}</div>)}{!selectedUnit.reservations.some(reservation => reservation.packageSelection) && <p>No package selections are recorded for this unit yet.</p>}<h3>Your additional supply requests</h3>{(data.merchandiseRequests ?? []).filter(item => item.unitId === selectedUnit.unitId && (item.unitKey === selectedUnit.key || selectedUnit.reservations.some(reservation => item.unitKey === `reservation:${reservation.id}`))).map(item => <article key={item.id} className="tenant-request"><strong>{currency(item.total)} · {formatSouthAfricaDate(item.createdAt)}</strong><p>Store follow-up: {item.task.status.replaceAll("_", " ").toLowerCase()}. This is a request, not a payment receipt.</p><ul>{packageItems(item.items).map((product, index) => <li key={index}>{product.quantity} × {product.name}</li>)}</ul><small>Reference {item.id}</small></article>)}{["ACTIVE", "DRAFT", "NOTICE_GIVEN"].includes(selectedUnit.status) && <a href="#tenant-shop">Request more packing supplies →</a>}</section>}
