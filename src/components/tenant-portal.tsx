  ...
  return <main className="tenant-portal">
    ...
    <div className="tenant-body">
      {checking ? <p role="status">Opening your secure space…</p> : !data ? <div className="tenant-entry">
        ...
        {selectedUnit && <section className="tenant-card tenant-move-in" aria-label="Your move-in">
          <h2>Your move-in</h2>
          ...
          {!!selectedUnit.reservations.length && <div id="access-photo" className="tenant-photo-destination" ref={photoSection} tabIndex={-1} aria-label={`Access photograph for Unit ${selectedUnit.number}`}>
            {selectedUnit.reservations.map(reservation => <TenantFacePhoto key={reservation.id} reservationId={reservation.id} />)}
          </div>}
        </section>}
        {data.testPayments?.some(p => p.accountId === effectiveAccountId) && <section className="tenant-card"><h2>Test payment activity</h2><p>Sandbox payments are testing records. They are not rent receipts and do not authorise key collection.</p>{data.testPayments.filter(p => p.accountId === effectiveAccountId).map(p => <p key={p.id}>{currency(p.amount, p.currency)} · {p.status === "TEST_SUCCEEDED" || p.status === "SUCCEEDED" ? "Test verified" : "Test pending"}</p>)}</section>}
