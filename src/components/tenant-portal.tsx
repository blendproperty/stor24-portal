  ...
  return <main className="tenant-portal">
    ...
    <div className="tenant-body">
      {checking ? <p role="status">Opening your secure space…</p> : !data ? <div className="tenant-entry">
        ...
        {selectedUnit && data.onboarding?.filter(item => selectedUnit.reservations.some(r => r.id === item.reservationId)).map(item => <section className="tenant-card" key={item.reservationId}><h2>{item.ready ? "Ready for key collection" : "Before key collection"}</h2><p>Your signed agreement is saved. No new signature is needed.</p><p>{currency(String(item.paidAmount))} verified towards {currency(String(item.requiredAmount))}. Agreed move-in: {item.startDate ?? "Awaiting confirmation"}.</p>{item.mandateStatus && <p>Debit-order mandate: {item.mandateStatus.replaceAll("_", " ")}. This is separate from payment; collections are not enabled.</p>}<ul>{item.blockers.map(blocker => <li key={blocker}>{blocker}</li>)}</ul><p>Staff confirm your identity and key handover at the guard house. Facial access is confirmed separately.</p></section>)}
        ...
        {selectedUnit && <section className="tenant-card"><h2>Facial access</h2><p>{selectedUnit.accessState === "ACTIVE" ? "Access is recorded as active. Contact the store if entry does not work." : "Facial access is not active. Your photo needs staff review, followed by identity checks and key handover before the access system can be activated."}</p></section>}
