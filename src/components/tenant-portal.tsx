  ...
  return <main className="tenant-portal">
    ...
    <div className="tenant-body">
      {checking ? <p role="status">Opening your secure space…</p> : !data ? <div className="tenant-entry">
        ...
        {selectedUnit && <TenantPurchases unitKey={selectedUnit.key} unitId={selectedUnit.unitId} unitNumber={selectedUnit.number} accountId={selectedUnit.accountId} canShop={selectedUnit.status === "ACTIVE" && !!selectedUnit.accountId} />}
        {selectedUnit && <details className="tenant-card"><summary>Earlier booking selections and supply requests</summary><p>Your recorded package selections. Reserved items are not proof of payment or delivery.</p>{selectedUnit.reservations.map(reservation => reservation.packageSelection && <div className="tenant-card" key={reservation.id}><h3>{reservation.packageSelection.packageName}</h3><p>{reservation.packageSelection.status.replaceAll("_", " ")} · {currency(reservation.packageSelection.priceSnapshot)} · {reservation.publicReference}</p><ul>{packageItems(reservation.packageSelection.itemsSnapshot).map((item, index) => <li key={index}>{item.quantity} × {item.name}</li>)}</ul>{reservation.packageSelection.fulfilledAt && <p>Fulfilled {formatSouthAfricaDate(reservation.packageSelection.fulfilledAt)}</p>}</div>)}{!selectedUnit.reservations.some(reservation => reservation.packageSelection) && <p>No package selections are recorded for this unit yet.</p>}<h3>Earlier requests — not paid orders</h3>{(data.merchandiseRequests ?? []).filter(item => item.unitId === selectedUnit.unitId && (item.unitKey === selectedUnit.key || selectedUnit.reservations.some(reservation => item.unitKey === `reservation:${reservation.id}`))).map(item => <article key={item.id} className="tenant-request"><strong>{currency(item.total)} · {formatSouthAfricaDate(item.createdAt)}</strong><p>Store follow-up: {item.task.status.replaceAll("_", " ").toLowerCase()}. This is a request, not a payment receipt.</p><ul>{packageItems(item.items).map((product, index) => <li key={index}>{product.quantity} × {product.name}</li>)}</ul><small>Reference {item.id}</small></article>)}</details>}
