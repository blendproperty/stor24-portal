...
type Document = { id: string; version: number; status: string; pageCount: number; documentType: string; expiresAt: string; erasedAt: string | null; replacementReason: string | null; reservation: { publicReference: string | null; facility: { name: string }; unit: { number: string }; customer: { firstName: string | null; lastName: string | null; companyName: string | null } } };
...
export function IdentityReview({ enabled }: { enabled: boolean }) {
  ...
  const clear = useCallback(() => { requestNumber.current++; if (imageUrl.current) URL.revokeObjectURL(imageUrl.current); imageUrl.current = ""; setPreview(""); }, []);
  const load = useCallback(async () => {
    clear(); setSelected(null); setViewed([]); setError("");
    try {
      const response = await fetch("/api/v1/identity-documents", { cache: "no-store" }), body = await response.json();
  ...
  useEffect(() => {
    ...
    fetch("/api/v1/identity-documents", { cache: "no-store" }).then(async response => {
      const body = await response.json();
      if (!response.ok) throw new Error(body.error?.message ?? "Unable to load identity reviews.");
      if (active) setDocuments(body.data);
  ...
  async function openPage(document: Document, page: number) {
    ...
    try {
      ...
      imageUrl.current = URL.createObjectURL(blob); setPreview(imageUrl.current); setViewed(current => [...new Set([...current, page])]);
  ...
  async function review(decision: "ACCEPT" | "REPLACE") {
    if (!selected) return; setBusy(true); setError(""); clear();
    try {
  ...
  return <main className="identity-review">
    ...
    <div className="identity-review__layout"><section aria-label="Identity documents"><h2>Recent submissions</h2>{!documents.length && <div className="identity-review__empty"><strong>No documents to review</strong><p>Customer uploads will appear here when collection is enabled.</p></div>}{documents.map(document => <button className={`identity-review__row ${selected?.id === document.id ? "is-selected" : ""}`} disabled={busy} key={document.id} onClick={() => { clear(); setSelected(document); setViewed([]); }}><span><strong>{document.reservation.customer.companyName || [document.reservation.customer.firstName, document.reservation.customer.lastName].filter(Boolean).join(" ") || "Customer"}</strong><small>{document.reservation.facility.name} · Unit {document.reservation.unit.number}</small></span><span className="identity-review__status">{labels[document.status] ?? document.status}</span></button>)}<p className="identity-review__fine">Showing the 200 most recent submissions within your permitted stores.</p></section>
