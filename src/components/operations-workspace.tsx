...
type Product = { id: string; facilityId: string; sku: string; name: string; category: string; barcode: string | null; costPrice: string; quantityOnHand: number; quantityReserved: number; reorderPoint: number; sellingPrice: string; active: boolean; facility: { name: string } };
type StoragePackage = { id: string; facilityId: string; code: string; name: string; description: string; badge: string | null; sellingPrice: string; minUnitAreaSqM: string | null; maxUnitAreaSqM: string | null; sortOrder: number; active: boolean; facility: { name: string }; items: Array<{ id: string; quantity: number; product: Product }> };
...
export function OperationsWorkspace() {
  ...
  async function updatePackage(formData: FormData) {
    ...
    const response = await fetch(`/api/v1/operations/storage-packages/${selectedPackage.id}`, {
      ...
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        code: formData.get("code"), name: formData.get("name"), description: formData.get("description"), badge: formData.get("badge") || undefined,
  ...
  return <div className="page-stack">
    ...
    {selectedProduct ? <div className="modal-backdrop"><div className="modal-card" role="dialog" aria-modal="true"><p className="eyebrow">Product detail</p><h2>{selectedProduct.name}</h2><p className="panel-subtitle">{selectedProduct.facility.name} · {selectedProduct.quantityOnHand} on hand · {selectedProduct.quantityReserved} reserved</p><form action={updateProduct} className="invite-form"><label>SKU<input name="sku" required maxLength={60} defaultValue={selectedProduct.sku}/></label><label>Product name<input name="name" required maxLength={160} defaultValue={selectedProduct.name}/></label><label>Category<input name="category" required defaultValue={selectedProduct.category}/></label><label>Barcode<input name="barcode" defaultValue={selectedProduct.barcode ?? ""}/></label><div className="form-grid two"><label>Cost price<input name="costPrice" type="number" min="0" step="0.01" defaultValue={selectedProduct.costPrice} required/></label><label>Selling price<input name="sellingPrice" type="number" min="0" step="0.01" defaultValue={selectedProduct.sellingPrice} required/></label><label>Reorder point<input name="reorderPoint" type="number" min="0" step="1" defaultValue={selectedProduct.reorderPoint} required/></label><label className="checkbox-row"><input name="active" type="checkbox" defaultChecked={selectedProduct.active}/> Active product</label></div><p className="panel-subtitle">Use Move stock for receipts, damage, returns and audited quantity corrections.</p><div className="form-actions"><button type="button" className="button button-secondary" onClick={() => setSelectedProduct(null)}>Cancel</button><button className="button button-primary" disabled={busy}>{busy ? "Saving…" : "Save product"}</button></div></form></div></div> : null}
    ...
    {selectedPackage ? <div className="modal-backdrop"><div className="modal-card modal-card-wide" role="dialog" aria-modal="true"><p className="eyebrow">Package detail</p><h2>Edit {selectedPackage.name}</h2><p className="panel-subtitle">{selectedPackage.facility.name} · Existing reservations keep their original price and contents snapshot.</p><form action={updatePackage} className="invite-form"><div className="form-grid two"><label>Internal code<input name="code" required defaultValue={selectedPackage.code}/></label><label>Public name<input name="name" required defaultValue={selectedPackage.name}/></label><label>Badge<input name="badge" defaultValue={selectedPackage.badge ?? ""}/></label><label>Once-off price<input name="sellingPrice" type="number" min="0" step="0.01" required defaultValue={selectedPackage.sellingPrice}/></label><label>Minimum unit area m²<input name="minUnitAreaSqM" type="number" min="0.1" step="0.1" defaultValue={selectedPackage.minUnitAreaSqM ?? ""}/></label><label>Maximum unit area m²<input name="maxUnitAreaSqM" type="number" min="0.1" step="0.1" defaultValue={selectedPackage.maxUnitAreaSqM ?? ""}/></label><label>Display order<input name="sortOrder" type="number" min="0" step="1" defaultValue={selectedPackage.sortOrder}/></label><label className="checkbox-row"><input name="active" type="checkbox" defaultChecked={selectedPackage.active}/> Available to booking and sales channels</label></div><label>Description<textarea name="description" minLength={10} maxLength={500} required rows={3} defaultValue={selectedPackage.description}/></label><fieldset className="package-builder"><legend>Package contents</legend>{data?.products.filter((product) => product.facilityId === selectedPackage.facilityId).map((product) => <label key={product.id}><span><strong>{product.name}</strong><small>{product.quantityOnHand - product.quantityReserved} available · R {Number(product.sellingPrice).toFixed(2)}</small></span><input name={`quantity:${product.id}`} type="number" min="0" step="1" defaultValue={selectedPackage.items.find((item) => item.product.id === product.id)?.quantity ?? 0} aria-label={`${product.name} quantity`}/></label>)}</fieldset><div className="form-actions"><button type="button" className="button button-secondary" onClick={() => setSelectedPackage(null)}>Cancel</button><button className="button button-primary" disabled={busy}>{busy ? "Saving…" : "Save package"}</button></div></form></div></div> : null}
  </div>;
}
