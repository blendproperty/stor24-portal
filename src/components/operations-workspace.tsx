...
export function OperationsWorkspace() {
  ...
  return <div className="page-stack">
    ...
    <section className="dashboard-grid">
      ...
      <article className="panel"><div className="hub-heading"><div><h2>End-of-day control</h2><p>Closed periods and recorded cash variance.</p></div><RefreshCw size={20}/></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Date</th><th>Facility</th><th>Status</th><th>Variance</th></tr></thead><tbody>{data?.dailyCloses.length ? data.dailyCloses.map((close) => <tr key={close.id}><td>{new Date(close.businessDate).toLocaleDateString("en-ZA")}</td><td>{close.facility.name}</td><td><StatusPill tone="positive">{close.status}</StatusPill></td><td>{close.variance ?? "—"}</td></tr>) : <tr><td colSpan={4} className="empty-cell">No daily closes recorded.</td></tr>}</tbody></table></div></article>
    </section>
