...
import { DailyCloseHistory, type DailyCloseRecord } from "@/components/daily-close-history";
...
export function OperationsWorkspace({ view = "operations" }: { view?: "operations" | "merchandise" }) {
  ...
  const packageCreationRequest = useRef(false);
  const [packageCreationMessage, setPackageCreationMessage] = useState("");
  const [packageCreationUncertain, setPackageCreationUncertain] = useState(false);
  async function createInventory(kind: "storagePackage", payload: Record<string, unknown>) {
    if (packageCreationRequest.current || packageCreationUncertain) return false;
    packageCreationRequest.current = true;
    setBusy(true); setPackageCreationMessage("");
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 20_000);
    try {
      const response = await fetch("/api/v1/operations", { method: "POST", signal: controller.signal, headers: { "content-type": "application/json" }, body: JSON.stringify({ kind, payload }) });
      const body = await response.json();
      if ([400, 401, 403, 404, 422, 429].includes(response.status)) {
        setPackageCreationMessage(typeof body.error?.message === "string" ? body.error.message : "Package was not created. Check the details and your access, then try again.");
        return false;
      }
      const saved = body.data;
      const items = payload.items as Array<{productId: string; quantity: number}>;
      const matchingItems = Array.isArray(saved?.items) && saved.items.length === items.length && items.every(item => saved.items.filter((entry: {productId: string; quantity: number}) => entry.productId === item.productId && entry.quantity === item.quantity).length === 1);
      if (!response.ok || typeof saved?.id !== "string" || !saved.id || saved.facilityId !== payload.facilityId || saved.code !== payload.code || saved.name !== payload.name || Number(saved.sellingPrice) !== payload.sellingPrice || saved.active !== true || !matchingItems) throw new Error("Unconfirmed package creation");
      setPackageCreationMessage("Package created.");
      await load(); return true;
    } catch {
      setPackageCreationUncertain(true);
      setPackageCreationMessage("We could not confirm whether this package was created. Reload the catalogue and check the package contents and price before creating it again.");
      return false;
    } finally {
      clearTimeout(timeout); packageCreationRequest.current = false; setBusy(false);
    }
  }
  ...
  const productUpdateRequest = useRef(false);
  const [productUpdateMessage, setProductUpdateMessage] = useState("");
  const [productUpdateUncertain, setProductUpdateUncertain] = useState(false);
  async function updateProduct(formData: FormData) {
    if (!selectedProduct || productUpdateRequest.current || productUpdateUncertain) return;
    productUpdateRequest.current = true;
    setBusy(true); setProductUpdateMessage("");
    const id = selectedProduct.id;
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 20_000);
    const input = { sku: String(formData.get("sku") ?? "").trim(), name: String(formData.get("name") ?? "").trim(), category: String(formData.get("category") ?? "").trim(), barcode: formData.get("barcode") || undefined, imageUrl: formData.get("imageUrl") || null, costPrice: Number(formData.get("costPrice")), sellingPrice: Number(formData.get("sellingPrice")), reorderPoint: Number(formData.get("reorderPoint")), active: formData.get("active") === "on" };
    try {
      const response = await fetch(`/api/v1/operations/products/${id}`, { method: "PATCH", signal: controller.signal, headers: { "content-type": "application/json" }, body: JSON.stringify(input) });
      const body = await response.json();
      if ([400, 401, 403, 404, 422, 429].includes(response.status)) {
        setProductUpdateMessage(typeof body.error?.message === "string" ? body.error.message : "Product was not updated. Check the details and your access, then try again.");
        return;
      }
      const saved = body.data;
      if (!response.ok || saved?.id !== id || saved.facilityId !== selectedProduct.facilityId || saved.sku !== input.sku || saved.name !== input.name || saved.category !== input.category || Number(saved.costPrice) !== input.costPrice || Number(saved.sellingPrice) !== input.sellingPrice || saved.reorderPoint !== input.reorderPoint || saved.active !== input.active) throw new Error("Unconfirmed product update");
      setSelectedProduct(null);
      setProductUpdateMessage("Product updated.");
      await load();
    } catch {
      setProductUpdateUncertain(true);
      setProductUpdateMessage("We could not confirm the product changes. Reload the catalogue and check the details before editing again.");
    } finally {
      clearTimeout(timeout); productUpdateRequest.current = false; setBusy(false);
    }
  }
  ...
  const merchandiseModals = <>
    ...
    {selectedProduct ? <ProductEditorModal product={selectedProduct} busy={busy} message={productUpdateMessage} uncertain={productUpdateUncertain} close={() => setSelectedProduct(null)} save={updateProduct}/> : null}
  ...
  return <div className="page-stack">
    ...
    <section className="dashboard-grid">
      <article className="panel daily-close-panel"><div className="hub-heading"><div><h2>End-of-day control</h2><p>Recorded close snapshots and cash variance.</p></div><RefreshCw size={20}/></div><DailyCloseForm facilities={(data?.facilities ?? []).filter(facility => data?.dailyCloseFacilityIds?.includes(facility.id))} unavailable={loading || readFailed} onRecorded={load}/><DailyCloseHistory records={data?.dailyCloses ?? []}/></article>
    </section>
