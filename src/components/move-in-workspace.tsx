...
export function MoveInWorkspace({
  ...
  const floors = useMemo(
    () =>
      [...new Set(available.map((unit) => unit.floor).filter(Boolean))].sort(
        (a, b) =>
          a.localeCompare(b, "en-ZA", { numeric: true, sensitivity: "base" }),
      ),
    [available],
  );
  ...
  const groups = useMemo(() => {
    const counts = new Map<
      string,
      { type: string; measure: string; count: number }
    >();
    available.forEach((unit) => {
      const measure =
        filterMode === "area"
          ? (unit.area?.toFixed(1) ?? "Not set")
          : unit.width && unit.length
            ? `${unit.width.toFixed(1)} × ${unit.length.toFixed(1)} m`
            : "Not set";
    ...
    const measure =
      filterMode === "area"
        ...
        : unit.width && unit.length
          ...
          : "Not set";
    return (
      (filterKey === "ALL" || filterKey === `${unit.typeName}|${measure}`) &&
  ...
  async function addCustomer(formData: FormData) {
    ...
    const response = await fetch("/api/v1/leasing/customers", {
      ...
      body: JSON.stringify({
        ...
        communicationConsent: {
          email: formData.get("emailConsent") === "on",
          sms: formData.get("smsConsent") === "on",
          whatsapp: formData.get("whatsappConsent") === "on",
          phone: false,
    ...
    if (!response.ok) {
      setCustomerError(
        payload.error?.message ?? "Customer could not be created.",
      );
      return;
    }
    const name =
    ...
    setCustomerOptions((current) => [
      { id: payload.data.id, name, email: payload.data.email ?? null },
      ...current,
    ]);
    setCustomerId(payload.data.id);
        ...
        <section className="unit-selector-layout">
          <article className="panel unit-results">
            <div className="unit-toolbar">
              ...
              <strong>{visible.length} units</strong>
            </div>
                ...
                <tbody>
                  {visible.map((unit) => (
                    ...
                    >
                      ...
                      <td>{unit.area?.toFixed(1) ?? "—"} m²</td>
                      <td>
                        R{" "}
                        {unit.monthlyRate.toLocaleString("en-ZA", {
                          minimumFractionDigits: 2,
                        })}
                      </td>
                      <td>{unit.floor || "—"}</td>
                      <td>{unit.features.join(", ") || unit.zone || "—"}</td>
                      <td>
                        <StatusPill
                          tone={
                            unit.status === "AVAILABLE" ? "positive" : "warning"
                          }
                        >
                          {unit.status === "AVAILABLE" ? "Vacant" : "Reserved"}
                        </StatusPill>
                      </td>
                    </tr>
                  ))}
          ...
          <aside className="unit-filter-panel">
            <section className="panel">
              <h3>Selected unit</h3>
              <strong className="selected-unit-number">
            ...
            <section className="panel">
              ...
              <p>
                {selected
                  ? `${selected.typeName}, ${selected.area?.toFixed(1) ?? "area not set"} m² at R ${selected.monthlyRate.toLocaleString("en-ZA")} per month.`
        ...
        <section className="panel panel-spacious">
          <form action={action} className="move-in-form">
            <input
              type="hidden"
              name="facilityId"
              value={selected?.facilityId ?? facilityId}
            />
            <input type="hidden" name="unitId" value={selectedId} />
            <label>
              Selected unit
              <input
                value={
                  selected
                    ? `${selected.number} · ${selected.typeName} · R ${selected.monthlyRate.toLocaleString("en-ZA")}`
                    : ""
                }
                readOnly
              />
            ...
            <label>
              ...
              <select
                ...
                required
              >
                <option value="">Select and confirm payment method</option>
      ...
      {showCustomer ? (
        <div className="modal-backdrop">
          <div className="modal-card" role="dialog" aria-modal="true">
            ...
            <form action={addCustomer} className="invite-form">
              <label>
                Customer type
