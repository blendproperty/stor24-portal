...
export function FacialPhotoQueue({ photos, policyConfigured, manageableFacilities }: { photos: FacialPhotoRow[]; policyConfigured: boolean; manageableFacilities: string[] | null }) {
  ...
  return <section className="face-workspace" aria-label="Private facial photo queue">
    ...
    <div className="face-steps"><article><Camera size={21} /><span>01 · Submit</span><h3>Customer photograph</h3><p>{policyConfigured ? "Approved consent and a verified booking are required." : "Photo collection is disabled: an approved consent and retention policy has not been configured."}</p></article><article><Check size={21} /><span>02 · Review</span><h3>Staff confirmation</h3><p>Review the photo, then confirm identity and move-in through key handover.</p></article><article><LockKeyhole size={21} /><span>03 · Activate</span><h3>Provider verification</h3><p>Automatic activation from this photo queue is not connected to Hikvision yet. An approved photo does not grant gate access.</p></article></div>
    ...
    <footer className="face-footer"><strong>What still needs to be completed</strong><p>Configure the approved photo policy, complete and verify the Hikvision activation connection, then test entry, suspension, restoration and removal on site. This page shows setup readiness, not a live gate status.</p></footer>
  </section>;
}
