...
export type FacialPhotoRow = { id: string; version: number; status: string; expiresAt: string; customerName: string; facilityId: string; facilityName: string; unitNumber: string };
...
export function FacialPhotoQueue({ photos, policyConfigured, manageableFacilities }: { photos: FacialPhotoRow[]; policyConfigured: boolean; manageableFacilities: string[] | null }) {
  ...
  const [preview, setPreview] = useState<{ id: string; version: number; url: string; name: string; expiresAt: string } | null>(null);
  const [busy, setBusy] = useState(false), [message, setMessage] = useState("");
  const [now, setNow] = useState(() => Date.now());
  useEffect(() => { const timer = setInterval(() => setNow(Date.now()), 1000); return () => clearInterval(timer); }, []);
  useEffect(() => {
    if (!preview) return;
    const timer = setTimeout(() => setPreview(null), Math.min(60_000, Math.max(0, new Date(preview.expiresAt).getTime() - Date.now())));
  ...
  return <section className="face-workspace" aria-label="Private facial photo queue">
    <header className="face-heading"><div><span className="face-eyebrow">MOVE-IN & ACCESS</span><h2>A reviewed photo.<br />A confirmed arrival.</h2><p>Prepare facial access from the customer’s private submission.</p></div><span className="face-pill"><LockKeyhole size={14} /> Activation on hold</span></header>
    <div className="face-steps"><article><Camera size={21} /><span>01 · Submit</span><h3>Customer photograph</h3><p>{policyConfigured ? "Approved consent and a verified booking are required." : "Collection is off while legal confirms consent and retention."}</p></article><article><Check size={21} /><span>02 · Review</span><h3>Staff confirmation</h3><p>Review the photo, then confirm identity and move-in through key handover.</p></article><article><LockKeyhole size={21} /><span>03 · Activate</span><h3>Provider verification</h3><p>Enrolment, gate entry and removal must be proven before live use.</p></article></div>
    ...
    {photos.length ? <div className="face-rows">{photos.map(photo => {
      const expired = new Date(photo.expiresAt).getTime() <= now, canManage = manageableFacilities === null || manageableFacilities.includes(photo.facilityId);
      const reviewable = !expired && photo.status === "WAITING_REVIEW";
      return <article className="face-row" key={`${photo.id}:${photo.version}`}><div><h4>{photo.customerName}</h4><p>{photo.facilityName} · Unit {photo.unitNumber}</p><small>Version {photo.version} · Retention ends {formatSouthAfricaDateTime(photo.expiresAt)} SAST</small></div><span className="face-row-status">{labels[expired && ["APPROVED", "WAITING_REVIEW", "PENDING_PROVIDER"].includes(photo.status) ? "EXPIRED" : photo.status] ?? "Review required"}</span><div className="face-row-actions">{canManage && reviewable && <><button className="button button-secondary" disabled={busy} onClick={() => void open(photo)}>View privately</button><button className="button button-primary" disabled={busy || preview?.id !== photo.id || preview.version !== photo.version} onClick={() => void review(photo, "APPROVE")}>Approve photo</button><button className="button button-secondary" disabled={busy} onClick={() => void review(photo, "REJECT")}>Request new photo</button></>}</div></article>;
    ...
    <footer className="face-footer"><strong>Before facial access can go live</strong><p>Approved privacy arrangements, verified provider enrolment and deletion, followed by an on-site entry, suspension, restoration and revocation test.</p></footer>
  </section>;
}
