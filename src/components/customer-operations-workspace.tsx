...
type PublicLease = { id: string; status: string; version: string; paymentMethod: string; signedAt: string | null; signedPdfSha256: string | null; mandate?: { status: string; reference: string; signedPdfSha256: string | null } | null };
type Customer = { id: string; type: string; firstName: string | null; lastName: string | null; companyName: string | null; email: string | null; phone: string | null; identityRef: string | null; taxNumber: string | null; dateOfBirth: string | null; billingAddress: Contact | null; alternateContact: Contact | null; workContact: Contact | null; emergencyContact: Contact | null; communicationConsent: { email?: boolean; sms?: boolean; phone?: boolean; whatsapp?: boolean } | null; notes: string | null; tenancies: Tenancy[]; leads: { id: string; stage: string; source: string; updatedAt: string }[]; reservations: { id: string; status: string; facility: { name: string }; unit: { number: string }; publicLease: PublicLease | null }[] };
  ...
  return <div className="page-stack customers-workspace"><PageHeader eyebrow="Operations centre" title="Customers & tenants" description="Operational customer records, contacts, consent, occupancy and activity across all permitted stores." action={<div className="form-actions"><Link href="/operations/move-in" className="button button-secondary">Move in</Link><button className="button button-primary" disabled={readBusy || !!readError} onClick={() => { setEditing("new"); setError(""); }}><Plus size={16}/>Add customer</button></div>}/>
    ...
    <section className="accounts-layout"><aside className="panel accounts-list"><label className="toolbar-search"><Search size={16}/><input placeholder="Search name, contact or ID" value={search} onChange={(event) => setSearch(event.target.value)}/></label>{visible.map((customer) => <button type="button" className={selectedId === customer.id ? "account-list-row active" : "account-list-row"} onClick={() => setSelectedId(customer.id)} key={customer.id}><span><strong>{nameOf(customer)}</strong><small>{customer.phone || customer.email || "No contact details"}</small></span>{customer.type === "BUSINESS" ? <Building2 size={17}/> : <UserRound size={17}/>}</button>)}{!visible.length ? <p className="empty-cell">No customer records found.</p> : null}</aside>
      <article className="panel panel-spacious customer-detail">{selected ? <><div className="panel-heading"><div><p className="eyebrow">{selected.type === "BUSINESS" ? "Business customer" : "Individual customer"}</p><h2>{nameOf(selected)}</h2><p className="panel-subtitle">{selected.email || "No email"} · {selected.phone || "No phone"}</p></div><button className="button button-secondary" onClick={() => { setEditing(selected); setError(""); }}><Pencil size={15}/>Edit details</button></div>
        ...
        <section className="customer-consent"><strong>Communication consent</strong><StatusPill tone={selected.communicationConsent?.email ? "positive" : "neutral"}>Email {selected.communicationConsent?.email ? "allowed" : "not allowed"}</StatusPill><StatusPill tone={selected.communicationConsent?.sms ? "positive" : "neutral"}>SMS {selected.communicationConsent?.sms ? "allowed" : "not allowed"}</StatusPill><StatusPill tone={selected.communicationConsent?.whatsapp ? "positive" : "neutral"}>WhatsApp {selected.communicationConsent?.whatsapp ? "allowed" : "not allowed"}</StatusPill><StatusPill tone={selected.communicationConsent?.phone ? "positive" : "neutral"}>Phone {selected.communicationConsent?.phone ? "allowed" : "not allowed"}</StatusPill></section>
        ...
        <h3>Signed booking agreements</h3><div className="table-wrap"><table className="data-table"><thead><tr><th>Store</th><th>Unit</th><th>Payment</th><th>Signed</th><th>Document</th></tr></thead><tbody>{selected.reservations.some((reservation) => reservation.publicLease?.status === "SIGNED") ? selected.reservations.filter((reservation) => reservation.publicLease?.status === "SIGNED").map((reservation) => <tr key={reservation.publicLease!.id}><td>{reservation.facility.name}</td><td>{reservation.unit.number}</td><td>{reservation.publicLease!.paymentMethod.replaceAll("_", " ")}</td><td>{reservation.publicLease!.signedAt ? new Date(reservation.publicLease!.signedAt).toLocaleString("en-ZA") : "—"}</td><td>{reservation.publicLease!.signedPdfSha256 ? <a className="button button-secondary" href={`/api/v1/public-leases/${reservation.publicLease!.id}/signed-pdf`}><Download size={14}/>Download signed PDF</a> : "Preparing PDF"}</td></tr>) : <tr><td colSpan={5} className="empty-cell">No signed booking agreement attached.</td></tr>}</tbody></table></div>
