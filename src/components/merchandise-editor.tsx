...
export function PackageEditorModal({ storagePackage, products, busy, close, save, mode = "edit" }: { storagePackage: MerchandisePackage; products: MerchandiseProduct[]; busy: boolean; close: () => void; save: (data: FormData) => void | Promise<void>; mode?: "create" | "edit" }) {
  ...
  const available = facilityProducts.filter((product) => !(quantities[product.id] > 0) && `${product.name} ${product.sku} ${product.category}`.toLowerCase().includes(query.toLowerCase()));
    ...
    <form onSubmit={submit} className="merch-editor-form">
      <div className="package-studio-grid">
        ...
        <section className="package-content-studio">
          ...
          <div className="catalogue-picker"><div className="catalogue-picker-heading"><div><h3>Add products</h3><p>Search by product, SKU or category.</p></div><label className="catalogue-search"><Search size={16}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search catalogue…"/></label></div><div className="catalogue-results">{available.map((product) => <button type="button" className="catalogue-product" key={product.id} onClick={() => setQuantity(product.id, 1)}><div className="product-thumb" style={product.imageUrl ? { backgroundImage: `url(${product.imageUrl})` } : undefined}>{!product.imageUrl ? <PackagePlus size={18}/> : null}</div><span><strong>{product.name}</strong><small>{product.quantityOnHand - product.quantityReserved} available · R {Number(product.sellingPrice).toFixed(2)}</small></span><Plus size={17}/></button>)}{!available.length ? <p className="catalogue-empty">No more matching products to add.</p> : null}</div></div>
        </section>
      </div>
