...
import { ArrowLeft, ArrowRight, Check, ImagePlus, Minus, PackagePlus, Plus, Search, Trash2, X } from "lucide-react";
...
function ImagePicker({ initialValue, label, title, badge }: { initialValue?: string | null; label: string; title?: string; badge?: string | null }) {
  ...
  return <div className="merch-image-picker">
    ...
    <div className={`merch-image-preview ${imageUrl ? "has-image" : ""}`} style={imageUrl ? { backgroundImage: `url(${imageUrl})` } : undefined}>
      ...
      {imageUrl && title ? <div className="package-art-overlay"><small>{badge}</small><strong>{title}</strong></div> : null}
    </div>
...
export function PackageEditorModal({ storagePackage, products, busy, close, save, mode = "edit", message, uncertain = false }: { message?: string; uncertain?: boolean; storagePackage: MerchandisePackage; products: MerchandiseProduct[]; busy: boolean; close: () => void; save: (data: FormData) => void | Promise<void>; mode?: "create" | "edit" }) {
  ...
  const [packageName, setPackageName] = useState(storagePackage.name);
  const [packageBadge, setPackageBadge] = useState(storagePackage.badge ?? "");
  const [showAll, setShowAll] = useState(false);
  function moveProducts(direction: number) {
    carousel.current?.scrollBy({ left: direction * Math.max(180, carousel.current.clientWidth * .8), behavior: "smooth" });
  }
