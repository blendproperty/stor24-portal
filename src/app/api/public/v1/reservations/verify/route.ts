...
export async function POST(request: Request) {
  ...
  if (await rateLimit(`public-verification:${privacyHash(request.headers.get("x-stor24-client-ip")?.slice(0, 128) || requestIp(request))}`, 15, 15 * 60 * 1000)) return Response.json({ error: { message: "Too many verification attempts. Try again later." } }, { status: 429 });
