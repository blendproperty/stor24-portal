...
const schema = z.object({ sessionId: z.string().min(10), checkoutToken: z.string().min(32), outcome: z.enum(["SUCCESS", "DECLINED", "CANCELLED", "TIMEOUT"]) });
export async function POST(request: Request) {
  ...
  const result = await completeSimulatedPayment(parsed.data.sessionId, parsed.data.checkoutToken, parsed.data.outcome);
