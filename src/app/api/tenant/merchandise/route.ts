...
import { merchandiseCheckoutEnabled, merchandiseCheckoutTestMode } from "@/lib/merchandise-checkout-access";
...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({ data: { checkoutEnabled, r10Test: merchandiseCheckoutTestMode(session), testAccess: checkoutEnabled && process.env.TENANT_MERCHANDISE_CHECKOUT_ENABLED !== "true", products: products.map(product => ({ id: product.id, name: product.name, imageUrl: product.imageUrl, category: product.category, price: product.sellingPrice, available: Math.max(0, product.quantityOnHand - product.quantityReserved) })) } }, { headers: tenantPrivateHeaders });
