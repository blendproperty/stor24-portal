...
export const customerSchema = z.object({ type: z.enum(["INDIVIDUAL", "BUSINESS"]).default("INDIVIDUAL"), firstName: z.string().trim().max(100).optional(), lastName: z.string().trim().max(100).optional(), companyName: z.string().trim().max(160).optional(), email: z.email().optional(), phone: z.string().trim().min(7).max(30).optional(), identityRef: z.string().trim().max(100).optional(), taxNumber: z.string().trim().max(100).optional(), dateOfBirth: z.coerce.date().optional(), billingAddress: contactRecord, emergencyContact: contactRecord, alternateContact: contactRecord, workContact: contactRecord, communicationConsent: z.object({ email: z.boolean().default(false), sms: z.boolean().default(false), phone: z.boolean().default(false), whatsapp: z.boolean().default(false), recordedAt: z.string().datetime().optional(), source: z.string().trim().max(80).optional() }).optional(), notes: z.string().trim().max(5000).optional() }).refine((v) => v.companyName || (v.firstName && v.lastName), "Provide a person or company name.");
...
export const moveInSchema = z.object({ reservationId: id.optional(), facilityId: id, customerId: id, unitId: id, startDate: z.coerce.date(), monthlyRate: money.optional(), initialCharge: money.default(0), accessState: z.string().trim().min(1).max(40).default("PENDING"), paymentMethod: z.enum(["DEBIT_ORDER", "CARD", "EFT", "OTHER"]) });
export const leaseSignatureSchema = z.object({ signerName: z.string().trim().min(2).max(120), initials: z.array(z.enum(LEASE_CLAUSE_KEYS)).refine((value) => LEASE_CLAUSE_KEYS.every((key) => value.includes(key)), "Every clause of the lease agreement must be initialled.") });
