...
try {
 for(const width of [1440,390,320]) {
  ...
  const before=posts;await page.getByRole('button',{name:'Reload close records',exact:true}).click();await expect(page.locator('table').getByText('CLOSED',{exact:true})).toBeVisible();assert.equal(posts,before);
  saved=null;mode='success';await page.reload();await open();await submit.click();await expect(page.getByRole('status')).toContainText('Daily close recorded');await expect(submit).toBeDisabled();await page.getByRole('button',{name:'Refresh close records',exact:true}).click();await expect(page.locator('table').getByText('CLOSED',{exact:true})).toBeVisible();assert.equal(posts,before+1);
