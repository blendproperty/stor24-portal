...
try {
 for(const width of [1440,390,320]) {
  ...
  await page.route('**/api/v1/reports/export?*',async route=>{
   assert.equal(route.request().method(),'GET');reads++;const url=new URL(route.request().url());assert.equal(url.searchParams.get('facilityId'),'a');
  ...
  const open=async(query='')=>{await page.goto(base+query);await page.getByLabel(/^Facility/).selectOption('a');};
  ...
  mode='success';const downloadEvent=page.waitForEvent('download');await button.click();const download=await downloadEvent;assert.equal(download.suggestedFilename(),'stor24-unit-availability-2026-09-01-2026-09-26.csv');assert.equal(await readFile(await download.path(),'utf8'),csv);await expect(page.getByRole('status')).toContainText('download prepared');assert.equal(downloads,1);
