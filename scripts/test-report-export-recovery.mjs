...
try {
 for(const width of [1440,390,320]) {
  ...
  await page.getByLabel(/^Report/).selectOption('unit-availability');await expect(page.getByLabel('From',{exact:true})).toHaveCount(0);await expect(page.getByLabel('To',{exact:true})).toHaveCount(0);await expect(page.getByText(/Current snapshot/)).toBeVisible();assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.screenshot({path:'output/report-export-recovery/snapshot-'+width+'.png'});const snapshotDownload=page.waitForEvent('download');await button.click();assert.equal((await snapshotDownload).suggestedFilename(),'stor24-unit-availability-current.csv');
