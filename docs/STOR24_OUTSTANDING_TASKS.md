...
| 5 | MRI Property Central finance integration | **Connection deployed; journal integration open** | Production-server and app-screen authentication/Blend database read verified; confirm the finance property and approved journal contract/mappings, then implement posting and reconciliation. See the 21 September MRI section below. |
...
**Status (21 September): encrypted setup, monthly ledger review and v2 connection checks deployed. Production-server authentication/discovery/Blend property read passed (HTTP200, zero visible properties). Blend selection is saved; live app sign-in/property read and reload persistence were verified at 14:31 SAST. Journal posting and reconciliation remain open.**
...
- [x] Saved the discovered Blend selection through the authenticated owner UI; verified app sign-in, property read, encrypted storage, safe audit and reload persistence.
- [ ] Confirm authorised test use and intended finance-property visibility separately; the successful Blend query returns zero properties.
...
The preparation workspace at `/billing/mri` stores credentials encrypted and reviews aggregate ledger source movements, with full-account test-history quarantine. A manual check authenticates with MRI and performs read-only discovery/property lookup. It never posts journals. The older v1 special-journal operation needs compatibility confirmation for the v2 account. See `docs/MRI_INTEGRATION.md` and canonical `PROJECT_CONTEXT.md` for exact implementation, validation and promotion evidence; this item remains open.
