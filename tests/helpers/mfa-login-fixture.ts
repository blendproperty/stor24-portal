...
export async function mfaLoginFixture(database: unknown, session: unknown = null) {
  ...
  const hooks = { getSession: async () => session, setSession: async (value: { sessionVersion: number }) => { sessions.push(value); }, clearSession: async () => {} };
  ...
  return { ...loaded.exports, jar, sessions };
}
